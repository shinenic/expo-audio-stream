...
function Recorder() {
  ...
  const uploadTasksRef = useRef<Array<FileSystem.UploadTask>>([]);

  const onAudioCallback = async (audio: AudioDataEvent) => {
    console.log("on Audio callback", audio);
    ...
    if (audio.chunkFileUri) {
      ...
      // Upload chunk to server if we have a session ID
      if (sessionIdRef.current) {
        try {
          const uploadUrl = `${serverUrl}/upload-chunk?id=${sessionIdRef.current}&index=${chunkCountRef.current}`;
          const uploadTask = FileSystem.createUploadTask(
            ...
            audio.chunkFileUri,
            {
              ...
              mimeType: "audio/wav",
              parameters: {
                filename: `chunk_${chunkCountRef.current}.wav`,
              },
            },
            (uploadProgress) => {
              const progress =
              ...
              setUploadStatus(
                `Uploading chunk ${chunkCountRef.current + 1}: ${Math.round(
                  progress * 100
                )}%`
              );
            }
          ...
          setUploadStatus(`Uploaded chunk ${chunkCountRef.current + 1}`);
          setUploadProgress(0);
        } catch (error) {
          console.error("Error uploading chunk:", error);
          setUploadStatus(
            `Failed to upload chunk ${chunkCountRef.current + 1}`
          );
          setUploadProgress(0);
        }
      }
  ...
  // Clean up upload tasks when component unmounts or recording stops
  useEffect(() => {
    return () => {
  ...
  const startRecording = async () => {
    ...
    try {
      ...
      const sessionResponse = await fetch(`${serverUrl}/start`, {
        method: "POST",
      });
      ...
      setUploadStatus("Server session started");
  ...
  const stopRecording = async () => {
    try {
      ...
      if (sessionIdRef.current) {
        setUploadStatus("Processing final audio...");
        const completeResponse = await fetch(
          `${serverUrl}/complete?id=${sessionIdRef.current}`,
          {
            method: "POST",
          }
        );
        ...
        setConcatUri(`${serverUrl}${url}`);
        setUploadStatus("Recording completed and processed");
      }
      ...
      setUploadStatus("Failed to stop recording");
      setUploadProgress(0);
    }
  };
  ...
  return (
    <View style={styles.recorderContainer}>
      ...
      <View style={styles.buttonGroup}>
        ...
        <Button
          onPress={playConcat}
          title="Play MP3 Recording (Server)"
          disabled={!concatUri}
        />
      </View>
