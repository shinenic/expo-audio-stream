...
export default class PipeTab extends React.Component {
  ...
  updateProgressDialog() {
    ...
    let completePercentage = Math.round(
      (timeInMilliseconds * 100) / totalVideoDuration
    );
    this.progressModalReference.current.update(
      `Creating video % ${completePercentage}`
    );
  }
    ...
    return (
      <View style={styles.screenStyle}>
        <View style={styles.headerViewStyle}>
          <Text style={styles.headerTextStyle}>FFmpegKit ReactNative</Text>
        </View>
        <View
          style={[
        ...
        <ProgressModal visible={false} ref={this.progressModalReference} />
        <Video
